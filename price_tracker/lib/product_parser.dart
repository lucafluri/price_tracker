...
class ProductParser{
  ...
  static bool validUrl(String url){
    if(isURL(url, { 'protocols': ['http','https'], 'require_tld': true, 'require_protocol': true, 'allow_underscores': true, 'host_whitelist': false, 'host_blacklist': false })){
  ...
  static Future<String> parseName(String url) async{
    ...
    switch(d){
      case "digitec.ch":
      case "galaxus.ch":
        String name = XPath.source(response.body).query("//*[@id='pageContent']/div/div[2]/div/div[2]/div/h1").get();
        final regexp = RegExp(r'[<]strong[>](.*)[<][\/]strong[>]\s*[<]span[>](.*)[<][\/]span[>]'); //Find first double
        final match = regexp.firstMatch(name);
        return match.group(1).replaceAllMapped(RegExp(r"<!--.*?-->"), (match) {return "";}).trim() + " " + match.group(2).trim();
        break;
    }
